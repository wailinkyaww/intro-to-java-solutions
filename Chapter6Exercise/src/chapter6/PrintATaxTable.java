...
public class PrintATaxTable {
	public static void main(String[] args) {
		...
		System.out.println("Income \t\t\t \t or Qualifying \t separeate \t a House");
		...
		while (taxableIncome <= 60000) {
			int single = (int) Math.round(computeTax(0, taxableIncome));
			int marriedJoint = (int) Math.round(computeTax(1, taxableIncome));
			int marriedSeparate = (int) Math.round(computeTax(2, taxableIncome));
			int headOfHouse = (int) Math.round(computeTax(3, taxableIncome));
			System.out.printf("%5d \t\t %5d \t\t %5d \t\t %5d \t\t %5d \n",(int) taxableIncome, single, marriedJoint, marriedSeparate,
					headOfHouse);
	...
	public static double computeTax(int status, double income) {
		...
		if (status == 0) { // Compute tax for single files
			if (income <= 8350)
				tax = income * 0.10;
